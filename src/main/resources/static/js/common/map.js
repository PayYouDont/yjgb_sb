...
function initEmer(){
	...
	for (var i = 0; i < emerArrayJson.length; i++) {
		...
		//1初始化应急信息
		var htmlOption='<div class="emerDiv" id="'+emerArrayJson[i].id+'" onclick="selectEmer(this)">'
						+'<div >'
						+'<span class="badge badge-success">急</span>'
						+'	<font id="divEmerName" class="emerTitle">'
								+emerArrayJson[i].emergencyName+'</font>'
						+'</div>'
						+'<div>'
						+'	<font class="emerFont1">等级：</font>'
						+'	<font id="divLevelName" class="emerFont2">'+emerArrayJson[i].accidentLevelName+'</font>'
						+'</div>'
						+'<div>'
						+'	<font class="emerFont1">类型：</font>'
						+'	<font id="divTypeName" class="emerFont2">'+emerArrayJson[i].accidentTypeName+'</font>'
						+'</div>'
						+'<div>'
						+'	<font class="emerFont1">开始：</font>'
						+'	<font  id="divStarttime" class="emerFont2">'+starttime+'</font>'
						+'</div>'
	    			  +'</div>';
...
function getChartsParm(emerId){
	$.post('../emergencyInfoAction/getDeviceByEmerId',{id:emerId},function(data){
		initBar(data);
		initPie(data);
		initMap(data);
	},'json');
...
//			devName:'',
		...
		frozenColumns:[[
			...
			{field:'emergencyName',title:'名称',width:120,align:'center',sortable:"true"},
        ]],
	    columns:[[
			{field:'accidentTypeName',title:'事件类型',width:150,align:'center',sortable:"true"},
	        {field:'accidentLevelName',title:'事件等级',width:150,align:'center',sortable:"true"}, 
	        {field:'start_time',title:'开始时间',width:150,align:'center',sortable:"true"},
	        {field:'end_time',title:'结束时间',width:150,align:'center'},
	        {field:'status',title:'状态',width:80,align:'center',sortable:"true",formatter:statusFormatter},
	        {field:'sound',title:'地址编码',width:150,align:'center',hidden:true}, 
	        {field:'lng',title:'经度',width:150,align:'center',hidden:true}, 
	        {field:'lat',title:'纬度',width:150,align:'center',hidden:true}, 
	    ]]    
	});
}
...
function doSearch(param){
	$("#mainTab").datagrid("load",{searchText:param});
}


function getDevByEmerAreaCode(addressCodes){
	$.post("../deviceInfoAction/getDevByCodeArray",{reJson:addressCodes},function(data){
