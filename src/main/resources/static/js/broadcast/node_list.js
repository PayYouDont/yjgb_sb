...
$(document).ready(function () {
    $('#mainTab').datagrid({
        ...
        singleSelect: false,
        idField: 'id',
        frozenColumns: [[{field: 'id', title: '节点id', width: 150, checkbox: true}, {
            field: 'name',
            title: '名称',
            width: 150,
            align: 'center'
...
function nodeStatusFormatter(value, rowData, rowIndex) {
    if (value == "0") {
        return '<span style="color: #FF9800;">停用</span>';
    } else if (value == "1") {
...
function linkStatusFormatter(value, rowData, rowIndex) {
    if (value >= 200 && value < 300) {
        ...
        return '<span style="color:red;">连接失败!错误代码:' + value + '</span>';
    }
}
/*搜索*/
function doSearch(value) {
    $('#mainTab').datagrid('load', {emergencyName: value});
}
/*页面刷新*/
