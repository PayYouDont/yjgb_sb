$(document).ready(function () {
    $('#mainTab').datagrid({
        ...
        checkOnSelect: true,
        singleSelect: false,
        /* onSelect:function(rowIndex,rowData){//选择表格行触发 getDetail(rowIndex,rowData); },*/
        idField: 'id',
        frozenColumns: [[{field: 'ck', width: 5, checkbox: true}, {
            title: 'id',
            field: 'id',
            width: 100,
            hidden: true
        }, {title: '指令类型', field: 'type', width: 150, align: 'center'}, {
            title: '指令tag',
            field: 'tag',
            width: 100,
            align: 'center'
        }, {
            title: '发送状态', field: 'status', width: 100, align: 'center', formatter: function (value) {
                if (value == -1) {
                    return '<span style="color: #FF9800;">未发送</span>';
                } else if (value == 0) {
                    return '<span style="color: green;">终端接收成功</span>';
                } else if (value == 13) {
                    return '<span style="color: red;">发送错误</span>';
                } else if (value == 60) {
                    return '<span style="color: red;">无法处理</span>';
                }
            }
        }, {title: '执行结果', field: 'result', width: 100, align: 'center'}]],
        columns: [[{title: '指令内容', field: 'cmdChar', align: 'left'}]]
    });
});
/*搜索*/
...
function deleteData(ids) {
    $.ajax({
        ...
        data: {ids: ids},
        beforeSend: function () {
            $.messager.progress({title: '系统提示', text: '请求中，请稍后。。。', interval: 300});
            ;
        },
    ...
    ;$.ajax({
        ...
        success: function (json) {
            ...
            if (json.success) {
                ...
                $.messager.alert('发送提示', '发送失败！', 'error');
            }
        },
        error: function (json) {
            $.messager.progress('close');
